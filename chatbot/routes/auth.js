...
router.post("/signup", async (req, res) => {
  ...
  try {
    const existing = await db.query("SELECT * FROM hm_users WHERE email = $1", [email]);
    if (existing.rows.length > 0) return res.status(400).json({ error: "User already exists" });

    const result = await db.query(
      "INSERT INTO hm_users (email, password, role) VALUES ($1, $2, $3) RETURNING *",
      [email, password, "user"]
    );
...
router.post("/login", async (req, res) => {
  ...
  try {
    const result = await db.query(
      "SELECT * FROM hm_users WHERE email = $1 AND password = $2",
      [email, password]
    );
