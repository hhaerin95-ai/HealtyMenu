...
pcsosWesternMeals.forEach(m => {
  db.get(
    `SELECT id FROM recipes WHERE name = ? AND bmi_category = ? AND cuisine_type = ?`,
    [m.name, m.bmi_category, m.cuisine_type],
    (err, row) => {
      if (row) {
        skipped++;
        console.log(`⏭️  [${m.bmi_category}][${m.cuisine_type}][PCOS:${m.is_pcos_friendly}] ${m.name} - Already exists`);
        return;
      }
      stmt.run(
        ...
        function(err) {
          if (err) {
            ...
            inserted++;
            console.log(`✅ [${m.bmi_category}][${m.cuisine_type}][PCOS:${m.is_pcos_friendly}] ${m.name}`);
          }
        }
      );
    }
